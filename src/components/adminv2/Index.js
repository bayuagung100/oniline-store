...
import { faAngleDown, faBars, faEye, faSignOutAlt } from '@fortawesome/free-solid-svg-icons';
                ...
                <div className="wrapper">
                    ...
                    <aside className="main-sidebar sidebar-dark-primary elevation-4">
                        <Link className="brand-link" to="/admin" onClick={this.active}>
                            <span className="brand-text font-weight-light">Welcome {this.state.detailLogin.firstname}</span>
                        </Link>
                        ...
                        <div className="sidebar">
                            ...
                            <nav className="mt-2">
                                <ul className="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                                    {
                                        this.state.detailLogin.role_id === 0 || this.state.detailLogin.role_id === 1 ? (
                                            MenuSide.map((value, index) => {
                                                return (
                                                    <li key={index} className={"nav-item " + (value.child !== undefined ? 'has-treeview' : '')}>
                                                        <Link to={`${match}` + value.url} onClick={this.active} className={"nav-link " + (value.active.includes(this.state.path) ? 'active' : '')}>
                                                            {value.fa}
                                                            <p>
                                                                {value.title}
                                                                        ...
                                                                        value.child.map((val, ind) => {
                                                                            return (
                                                                                <li key={ind} className="nav-item">
                                                                                    <Link to={`${match}` + val.url} onClick={this.active} className={"nav-link " + (this.state.path === val.active ? 'active' : '')}>
                                                                                        {val.fa}
                                                                                        <p>
                                                                                            {val.title}
                                                                                        </p>
                                                ...
                                                MenuSide.filter(obj => !obj.restricted).map((value, index) => {
                                                    return (
                                                        <li key={index} className={"nav-item " + (value.child !== undefined ? 'has-treeview' : '')}>
                                                            <Link to={`${match}` + value.url} onClick={this.active} className={"nav-link " + (value.active.includes(this.state.path) ? 'active' : '')}>
                                                                {value.fa}
                                                                    ...
                                                                    <ul className="nav nav-treeview">
                                                                        {
                                                                            this.state.detailLogin.role_id === 0 || this.state.detailLogin.role_id === 1 ? (
                                                                                value.child.map((val, ind) => {
                                                                                    ...
                                                                                    value.child.filter(obj => !obj.restricted).map((val, ind) => {
                                                                                        return (
                                                                                            <li key={ind} className="nav-item">
                                                                                                <Link to={`${match}` + val.url} onClick={this.active} className={"nav-link " + (this.state.path === val.active ? 'active' : '')}>
                                                                                                    {val.fa}
